      ...
      <SearchContext.Provider value={this.state.filters}>
        ...
        <Grid container direction="column">
          <Grid container item xs={4} style={{ zIndex: 20 }}>
            <Sidebar drawerOpen={this.state.drawerOpen} toggleFilter={(filterKey: string) => {
              // TODO: Learn how to typescript this dynamically..
              if (filterKey === 'is-verified') {
                this.setState({
                  filters: { ...this.state.filters, 'is-verified': !this.state.filters['is-verified'] }
                });
              }

              if (filterKey === 'is-location-screening-patients') {
                this.setState({
                  filters: { ...this.state.filters, 'is-location-screening-patients': !this.state.filters['is-location-screening-patients'] }
                });
              }

              if (filterKey === 'is-location-collecting-specimens') {
                this.setState({
                  filters: { ...this.state.filters, 'is-location-collecting-specimens': !this.state.filters['is-location-collecting-specimens'] }
                });
              }

              if (filterKey === 'is-location-accepting-third-party-orders-for-testing') {
                this.setState({
                  filters: { ...this.state.filters, 'is-location-accepting-third-party-orders-for-testing': !this.state.filters['is-location-accepting-third-party-orders-for-testing'] }
                });
              }

              if (filterKey === 'is-location-only-testing-patients-that-meet-criteria') {
                this.setState({
                  filters: { ...this.state.filters, 'is-location-only-testing-patients-that-meet-criteria': !this.state.filters['is-location-only-testing-patients-that-meet-criteria'] }
                });
              }

              if (filterKey === 'is-location-by-appointment-only') {
                this.setState({
                  filters: { ...this.state.filters, 'is-location-by-appointment-only': !this.state.filters['is-location-by-appointment-only'] }
                });
              }
            }
